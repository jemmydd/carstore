...
import com.luoyanjie.mechanical.bean.dto.my.PublishInHpItemDTO;
import com.luoyanjie.mechanical.bean.dto.publish.*;
import com.luoyanjie.mechanical.bean.dto.user.UserDTO;
import com.luoyanjie.mechanical.bean.entity.*;
import com.luoyanjie.mechanical.bean.enumBean.*;
...
import com.luoyanjie.mechanical.dao.mapper.*;
...
public class PublishServiceImpl implements PublishService {
    ...
    public PublishSubmitDTO publish(PublishSubmitParam param, Boolean needCheckUser) {
        ...
        userDOMapper.addPublishCount(param.getUserId());
    ...
    public Boolean modifyPublish(PublishSubmitParam param, Boolean needCheckUser) {
        ...
        UserDO userDO = userDOMapper.selectByPrimaryKey(param.getUserId());
        if (needCheckUser) checkUserForAddOrModify(userDO, "修改");
    ...
    public PublishDetailDTO getDetail(Integer userId, Integer publishId) {
        ...
        CollectionDO collectionDO = collectionDOMapper.selectByUserAndPublish(userId, publishId);
        Map<Integer, String> cm = categoryService.allMap();
    ...
    private List<OperateDTO> getOperates(PublishDO row, Integer requestUserId, PublishCallSceneEnum publishCallSceneEnum) {
        ...
        switch (publishCallSceneEnum) {
            case MY_PUBLISH:
                if (Objects.equals(row.getUserId(), requestUserId)) {
